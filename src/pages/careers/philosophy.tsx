...
export default function TalentPage() {
  ...
  // Custom talent data for the two centered cards
  const centeredTalents = [
    {
      key: "communication",
      title: lang === "KOR" ? "소통의 힘" : "The Power of Communication",
      desc: lang === "KOR" ? "열린 마음으로 소통하고\n팀워크를 중시하는 인재" : "Talents who communicate with an open mind\nand value teamwork",
      bgImage: "/images/communication_talent.jpg" // You'll need to add this image
    },
    {
      key: "passion",
      title: lang === "KOR" ? "열정의 에너지" : "Energy of Passion",
      desc: lang === "KOR" ? "자신의 일에 몰입하고\n뜨거운 열정으로 성과를 내는 인재" : "Talents who immerse themselves in their work\nand achieve results with burning passion",
      bgImage: "/images/passion_talent.jpg" // You'll need to add this image
    }
  ];
  ...
  return (
    <>
      ...
      <Layout>
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 md:gap-10 w-full max-w-4xl mx-auto">
                {centeredTalents.map((talent) => (
                  ...
                  >
                    <TalentCard
                      traitData={{ title: talent.title, desc: talent.desc }}
                      bgImage={talent.bgImage}
                    />
                  </motion.div>
                ))}
              </div>
        ...
        <hr className="my-6 sm:my-8 md:my-10 border-gray-200 w-full" />
      </Layout>
    </>
  );
}